...
import { Bell, Shield, User, Award, HelpCircle, UserCheck } from "lucide-react";
    ...
    <header className="flex flex-col md:flex-row items-stretch md:items-center justify-between px-4 md:px-8 py-4 bg-white border-b border-slate-200 gap-4">
      ...
      {currentUser && (
        <div className="flex flex-wrap items-center gap-4 md:gap-6">
          ...
          {/* SECTION DES RÔLES SÉCURISÉE */}
          <div className="flex items-center bg-slate-100 p-1 rounded-lg">
            {currentUser.role === UserRole.ADMIN ? (
              // Si c'est l'admin, on garde l'accès complet et les boutons de bascule
              <>
                <button
                  id="role-agent-btn"
              ...
              // Si c'est un Client, on enlève le bouton Agent et on affiche un badge fixe
              <span className="px-3 py-1 text-xs font-extrabold bg-white text-emerald-700 rounded-md shadow-sm flex items-center gap-1.5">
                <User className="w-3 h-3" /> Espace Client
              </span>
            ) : (
              // Si c'est un Agent, on enlève le bouton Client et on affiche un badge fixe
              <span className="px-3 py-1 text-xs font-extrabold bg-white text-emerald-700 rounded-md shadow-sm flex items-center gap-1.5">
                <UserCheck className="w-3 h-3" /> Espace Agent
              </span>
            )}
          </div>
